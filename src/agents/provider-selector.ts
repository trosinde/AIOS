...
export class ProviderSelector {
  ...
  /** Find cheapest available provider with the required capability */
  select(capability: string): { name: string; provider: LLMProvider } | undefined {
    const candidates = Object.entries(this.configs)
      ...
      .sort(([, a], [, b]) => (a.cost_per_mtok ?? 0) - (b.cost_per_mtok ?? 0));
